...
export default function ProductDetail() {
  ...
  const {description, name, originalPrice, isPromotion, isShipping, shortDescription, category, salePrice, promotionPercent} = data;
  
  // useEffect(() => {
  //   const fetchData = async () => {
  //     const resData = await productApi.getById(id);
  //     console.log(resData);
  //     setProduct(resData);
  //     // return resData;
  //   }
      ...
      <div className="lg:w-4/5 mx-auto flex flex-wrap">
        ...
        {category?.name || ""}
          ...
          <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
            ...
            <div className="flex ml-6 items-center">
              <label className="w-20" htmlFor="">Number: </label>
              <input type="number" min="0" max="100" name="" id="" />
            </div>
          </div>
          <div className="flex">
          
            <span className="title-font font-medium text-2xl text-gray-900">{salePrice.toLocaleString() || ""} VND</span>
            <button onClick={addCartHandler} className="flex ml-auto text-white bg-blue-500 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 rounded">Add to cart</button>
            <button onClick={addWishlistHandler} className="rounded-full w-10 h-10 bg-gray-200 p-0 border-0 inline-flex items-center justify-center text-gray-500 ml-4">
              <svg fill="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} className="w-5 h-5" viewBox="0 0 24 24">
        ...
        <div id="fullDescription" className="mt-10">
        {/* {description || ""} */}
...
// Do can i define state in loader ???
export async function loader({params, request}) {
  console.log("object", {request, params});
