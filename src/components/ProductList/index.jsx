...
ProductList.propTypes = {
  productListData: PropTypes.arrayOf(PropTypes.object).isRequired,
...
export default function ProductList(props) {
  ...
  const {productListData, pagination} = props;
  return (
    <div className="">
            ...
            {productListData.map((product) => {
              return (
                <Grid key={product.id} item xs={3}>
                  <ProductItem productData={product}  mode="light"/> 
              </Grid>
              );
            } )}
        </Grid>
        <Pagination onPaginate={props.onPaginate} paginationData = {pagination}/>
    </div>
  )
}
